    ...
    def fit(self):
        ...
        for epoch in range(self.epochs):
            ...
            for input, output in batches:
                ...
                X_mask = self.X_mask.expand(input.shape[0], -1, -1, -1) if self.X_mask is not None else None

                if self.encode_only is True:
                    prediction = self.model(input, X_mask)
                else:
                    y_mask = self.y_mask.expand(input.shape[0], -1, -1, -1) if self.y_mask is not None else None
                    prediction = self.model(input, output, X_mask, y_mask)
            ...
            with torch.inference_mode():
                ...
                for input, output in test_batches:
                    ...
                    if self.encode_only is True:
                        ...
                        prediction = self.model.generate(input, X_mask)
